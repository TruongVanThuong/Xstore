...
<?php 
    ...
    if ( $element_options['account_off_canvas'] && !class_exists('WooCommerce') && is_user_logged_in()) {
    ...
    if ( $element_options['account_off_canvas'] || $element_options['is_customize_preview'] ) {
	    ...
	    if ( 'yes' === get_option( 'woocommerce_enable_myaccount_registration' ) && get_option( 'woocommerce_registration_generate_password' ) == 'no'){
